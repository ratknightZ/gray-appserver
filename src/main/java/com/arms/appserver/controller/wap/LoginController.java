...
public class LoginController {
    ...
    @ResponseBody
    @RequestMapping(value = "/wap/register", method = {RequestMethod.POST,RequestMethod.GET})
...
//                }
                ...
                User user1 = userService.getByCellphone(account);
                if (user1 != null) {
                    UserVO userVO = new UserVO();
                    userVO.setUserId(user.getId());
                    userVO.setNick(user.getNick());
        ...
        try {
            userSourceService.add(user.getId(),source);
        } catch (UserException e) {
            logger.info("记录用户来源失败",e);
        }
        try {
    ...
    @ResponseBody
    @RequestMapping("/wap/sms_code_login")
